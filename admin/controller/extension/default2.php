...
class ControllerExtensionDefault2 extends Controller
{
    public function index()
    {
        ...
        $this->document->addStyle('view/stylesheet/default2/css/uikit-echothemes.min.css?v=2.8.0', 'stylesheet');
        ...
        $this->document->addStyle('view/stylesheet/default2/js/codemirror/codemirror.min.css?v=5.11', 'stylesheet');
        $this->document->addStyle('view/stylesheet/default2/default2.min.css?v=' . $this->language->get('ext_version'), 'stylesheet');
        ...
        $this->document->addScript('view/stylesheet/default2/js/codemirror/codemirror.min.js?v=5.11');
        $this->document->addScript('view/stylesheet/default2/js/codemirror/mode/css/css.min.js?v=5.11');
        $this->document->addScript('view/stylesheet/default2/js/codemirror/addon/display/autorefresh.min.js?v=5.11');
        $this->document->addScript('view/stylesheet/default2/js/codemirror/addon/selection/active-line.min.js?v=5.11');
        $this->document->addScript('view/stylesheet/default2/js/codemirror/addon/edit/matchbrackets.min.js?v=5.11');
        ...
        $data['custom_css'] = '';
        $custom_css_file    = DIR_CATALOG . 'view/theme/default2/stylesheet/build/default2-preset-' . $preset_id . '-custom.less';
        if (file_exists($custom_css_file)) {
            $data['custom_css'] = file_get_contents($custom_css_file);
        }
        ...
        // Setting
        $default_setting    = $this->model_extension_default2->defaultPreset();
        ...
        if (!$this->user->hasPermission('modify', 'extension/default2')) {
            ...
            if ($post['is_activate'] || isset($post['activate'])) {
                ...
                if (file_exists(DIR_APPLICATION . 'view/stylesheet/default2/lessc.inc.php')) {
                    ...
                    $less->compileFile($path . 'less/default2-preset.less', $path . 'build/default2-preset-' . $post['preset_id'] . '.min.css');

                    // Custom stylesheet
                    $file_full = $path . 'build/default2-preset-' . $post['preset_id'] . '-custom.less';
                    $file_min  = $path . 'build/default2-preset-' . $post['preset_id'] . '-custom.min.css';
                    if ($custom_css) {
                        ...
                        $less->compileFile($file_full, $file_min);
                    } else {
                        if (file_exists($file_full)) { unlink($file_full); }
                        if (file_exists($file_min)) { unlink($file_min); }
                    }
                }
            }
        }
